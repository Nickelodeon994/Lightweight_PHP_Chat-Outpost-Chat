...
function read_all_chats(): array {
    $all = [];
    $legacy = read_chats_from_file(CHATS_FILE);
    if ($legacy) {
        $all = array_merge($all, $legacy);
    }
    $files = glob(CHATS_DIR . '/*.json');
    if ($files) {
        sort($files, SORT_STRING);
        foreach ($files as $f) {
            $data = read_chats_from_file($f);
            if ($data) $all = array_merge($all, $data);
        }
    }
    usort($all, function($a,$b){ return (int)($a['ts'] ?? 0) <=> (int)($b['ts'] ?? 0); });
    return $all;
}

function append_chat_message(array $msg): bool {
    $path = current_hour_chats_file();
    $fp = fopen($path, 'c+');
    if (!$fp) return false;
    ...
    ftruncate($fp, 0);
    rewind($fp);
    $ok = fwrite($fp, json_encode($arr, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT)) !== false;
    fflush($fp);
    flock($fp, LOCK_UN);
    fclose($fp);
    return $ok;
    ...
    * {
    ...
    (function() {
        if (window.innerWidth > 768 && !/iPad|Android|Mobile/.test(navigator.userAgent)) {
            window.location.href = 'http://outpost.mcbes.top/';
        }
    })();
            ...
            try {
                ...
                if (initial) {
                    ...
                    reversedMessages.forEach(m => {
                        ...
                        seenIds.add(m.id);
                        list.appendChild(createBubble(m));
                    ...
                    for (let i = 0; i < data.messages.length; i++) {
                        ...
                        list.insertBefore(createBubble(m), list.firstChild);
                        seenIds.add(m.id);
                        latestTs = Math.max(latestTs, Number(m.ts || 0));
                    }
                }
...
if ($action === 'admin') {
    ...
    echo '<div><label>每单位限速时间允许发送消息条数</label><input class="input-text" type="number" name="rate_limit_count" value="'.e((string)($s['rate_limit_count'] ?? 10)).'" min="1" max="1000"></div>';
