...
function render_head(string $pageTitle) {
    ...
    ?>
    :root{
      --bg:#f6f8fb; --surface:#ffffff; --fg:#0b1220; --muted:#6b7280; --border:#e6eef8;
      --primary:#2563eb; --accent:#06b6d4; --danger:#ef4444; --ok:#16a34a;
      --bubble-self:#dbeafe; --bubble-other:#f1f5f9; --shadow: 0 8px 24px rgba(11,18,32,0.06);
      --nav-height:78px; --inputbar-height:72px;
    }
    html[data-theme="dark"]{
      --bg:#071018; --surface:#0f1720; --fg:#e6eef8; --muted:#9aa4b2; --border:#12202b;
      --primary:#7aa2ff; --accent:#22d3ee; --bubble-self:#0f2a4a; --bubble-other:#0b141a; --shadow: 0 8px 24px rgba(0,0,0,0.5);
    }
    *{box-sizing:border-box}
    * { -webkit-tap-highlight-color: transparent; -webkit-touch-callout: none; -webkit-user-select: none; user-select: none; }
    body{margin:0;background:var(--bg);color:var(--fg);font-family:Inter, "SF Pro Text", system-ui, -apple-system, "Segoe UI", Roboto, "Helvetica Neue", Arial;line-height:1.5}
    header{position:sticky;top:0;background:linear-gradient(180deg, rgba(255,255,255,0.1), rgba(255,255,255,0));backdrop-filter: blur(6px);border-bottom:1px solid var(--border);z-index:120}
    .brand{display:flex;align-items:center;gap:12px;padding:12px 16px}
    .logo{width:36px;height:36px;border-radius:10px;background:url('/Outpost.png') center/cover no-repeat;box-shadow:var(--shadow)}
    .app-title{font-weight:800;font-size:18px}
    .actions{margin-left:auto;display:flex;gap:8px;align-items:center}
    .icon-btn{width:40px;height:40px;border-radius:10px;border:1px solid var(--border);display:flex;align-items:center;justify-content:center;background:var(--surface);cursor:pointer}
    .icon-btn i {color: black; }
    html[data-theme="dark"] .icon-btn i {color: #d1d5db; }
    .center{min-height:calc(100vh - 64px);display:flex;align-items:center;justify-content:center;padding:16px}
    .card{background:var(--surface);border:1px solid var(--border);border-radius:14px;padding:20px;box-shadow:var(--shadow);width:100%;max-width:520px}
    .title-xl{font-size:20px;font-weight:700;margin-bottom:12px}
    .mobile-nav{position:fixed;left:0;right:0;bottom:0;background:var(--surface);border-top:1px solid var(--border);display:flex;justify-content:space-around;padding-top:6px;padding-bottom:6px;height:var(--nav-height);z-index:110}
    .mobile-nav a{display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;font-size:12px;color:var(--muted);text-decoration:none}
    .mobile-nav a .fa{font-size:18px;margin-bottom:4px}
    .mobile-nav a.active{color:var(--primary)}
    .chat-page{display:flex;flex-direction:column;height:calc(100vh - 64px)}
    .chat-top{padding:12px 16px;border-bottom:1px solid var(--border);background:var(--surface);display:flex;align-items:center;gap:12px}
    .chat-list{flex:1;overflow-y:auto;padding:16px 16px calc(var(--nav-height) + 16px) 16px;display:flex;flex-direction:column;gap:12px}
    .chat-inputbar{position:fixed;left:0;right:0;bottom:calc(var(--nav-height));padding:10px 12px;background:var(--surface);border-top:1px solid var(--border);z-index:115;height:var(--inputbar-height)}
    @media(min-width:840px){ .chat-inputbar{left:calc(50% - 480px);right:calc(50% - 480px);bottom:24px;border-radius:12px;box-shadow:var(--shadow)} }
    .row{display:flex;gap:10px;align-items:center;height:100%}
    .input-text{width:100%;padding:12px 48px 12px 12px;border-radius:12px;border:1px solid var(--border);background:var(--bg);color:var(--fg);outline:none;height:44px}
    .input-text:focus{box-shadow:0 6px 18px rgba(37,99,235,0.08);border-color:var(--primary)}
    .file-label{position:absolute;right:12px;top:50%;transform:translateY(-50%);cursor:pointer;color:var(--muted)}
    .file-label:hover{color:var(--primary)}
    .send-wrap{flex:0 0 20%}
    .send-btn{width:100%;padding:12px;border-radius:12px;border:none;background:var(--primary);color:#fff;cursor:pointer;font-weight:600;height:44px}
    .bubble{display:flex;gap:12px;align-items:flex-start}
    .avatar{width:44px;height:44px;border-radius:50%;object-fit:cover;background:#e6eef8;flex:none}
    .bubble-body{max-width:760px}
    ...
    .title-badge{font-size:12px;padding:3px 8px;border-radius:999px;background:rgba(37,99,235,0.08);color:var(--primary)}
    .text{margin-top:6px;padding:10px;border-radius:12px;background:var(--bubble-other);border:1px solid var(--border);white-space:pre-wrap}
    .self .text{background:var(--bubble-self)}
    .media{margin-top:8px}
    .img-thumb{width:100%;max-width:520px;height:auto;border-radius:12px;border:1px solid var(--border);cursor:zoom-in;object-fit:contain}
    .video-thumb{width:100%;max-width:520px;height:auto;border-radius:12px;border:1px solid var(--border);cursor:zoom-in}
    .profile{max-width:720px;margin:16px auto;display:flex;flex-direction:column;gap:16px}
    .profile-card{background:var(--surface);border:1px solid var(--border);border-radius:18px;padding:20px;box-shadow:var(--shadow)}
    .profile-top{display:flex;flex-direction:column;align-items:center;gap:12px}
    .profile-avatar{width:110px;height:110px;border-radius:50%;object-fit:cover;border:3px solid var(--primary);box-shadow:0 6px 18px rgba(0,0,0,0.12)}
    .profile-uid{font-size:13px;color:var(--muted)}
    .profile-form label{font-weight:600;margin-bottom:6px;display:block}
    .profile-form input{padding:12px;border-radius:12px;border:1px solid var(--border);background:var(--bg);width:100%}
    .table{width:100%;border-collapse:collapse}
    .table th,.table td{border-bottom:1px solid var(--border);padding:8px;text-align:left}
    .modal{position:fixed;inset:0;background:rgba(0,0,0,0.75);display:none;align-items:center;justify-content:center;z-index:200}
    .modal.active{display:flex}
    .modal-content{max-width:92vw;max-height:88vh}
    .modal img,.modal video{max-width:92vw;max-height:88vh;border-radius:12px}
    <?php
    echo '</style>';
    echo '<script>';
    ?>
